...
*Tables 8 and 9 show the most relevant results of Studies 1 and 4 where I investigated the comparative performance of SAM vs. traditional SEM estimation under correctly specified models.
...
create_styled_table <- function(data, caption_text, footnote_text) {
  ...
  data %>%
    kable("latex", booktabs = TRUE, longtable = TRUE,
          ...
          align = c("l", "c", "c", "c", "c", "c", "c", "c")) %>%  # Adjust alignments as needed
...
create_styled_table <- function(data, caption_text, footnote_text) {
  ...
  # Round numeric columns and remove underscores
  data <- data %>%
...
# Function to create a styled table for Study 3
create_styled_table <- function(data, caption_text, footnote_text) {
  data <- shorten_names(data)
  colnames(data) <- c("Method/Metric", "50", "100", "250", "500", "1000", "2500", "100000")
  data <- data %>%
  ...
  # Remove underscores from column names and values
  data <- data %>%
    dplyr::rename_all(~ gsub("_", " ", .)) %>%
    dplyr::mutate(across(everything(), ~ gsub("_", " ", .)))
  ...
  kbl(data,
      col.names = c("Method/Metric", "50", "100", "250", 
                    "500", "1000", "2500", "100000"), booktabs = TRUE, longtable = TRUE, 
      caption = caption_text) %>%
...
# Function to create a styled table for Study 4
create_styled_table <- function(data, caption_text, footnote_text) {
  data <- data %>%
    mutate(across(where(is.numeric), ~ round(., 3)))
...
create_styled_table <- function(data, caption_text, footnote_text) {
  ...
  kbl(data,
      col.names = c("Lambda", "phi = 0", "phi = 0.2", 
                    "phi = 0.4", "phi = 0.6", "phi = 0.8"), booktabs = TRUE, longtable = TRUE, 
      caption = caption_text) %>%
    kable_styling(full_width = FALSE, position = "left", font_size = 11) %>%
    row_spec(0, bold = TRUE) %>%
    kable_classic(full_width = FALSE) %>%
...
create_styled_table(bias_ci_s1b[["N_50"]][["LSAM_ML"]], caption_text1b, "LSAM ML = Local-SAM-maximum-likelihood estimation.")
...
# Format table for Study 4a
kbl(bias_ci_s4a[["DGM_1"]][["SEM_ML_metrics"]], 
    col.names = c("Beta", "50", "100", "250", "500", 
                  "1000", "2500", "100000"), booktabs = TRUE, longtable = TRUE, 
    caption = "Study 4a (Kosanke): Absolute bias of SEM-ML for DGM 2 (conditions with five positive unmodelled cross-loadings).") %>%
  kable_styling(full_width = FALSE, position = "left", font_size = 11) %>%
  add_header_above(c(" " = 1, "Sample size" = 7)) %>%
  row_spec(0, bold = TRUE) %>%
  kable_classic(full_width = FALSE) %>%
  footnote(general = "SEM ML = Maximum-likelihood estimation, SEM ULS = Unweighted-least-squares estimation, LSAM ML = Local-SAM-maximum-likelihood estimation, LSAM ULS = Local-SAM-unweighted-least-squares estimation, GSAM ML = Global-SAM-maximum-likelihood estimation, GSAM ULS = Global-SAM-unweighted-least-squares estimation.",
           general_title = "Note.",
           threeparttable = TRUE,
           escape = FALSE,
           footnote_as_chunk = TRUE)
...
# Format table for Study 4a (LSAM-ML)
kbl(bias_ci_s4a[["DGM_1"]][["LSAM_ML_metrics"]], 
    col.names = c("Beta", "50", "100", "250", "500", 
                  "1000", "2500", "100000"), booktabs = TRUE, longtable = TRUE, 
    caption = "Study 4a (Kosanke): Absolute bias of LSAM-ML for DGM 2 (conditions with five positive unmodelled cross-loadings).") %>%
  kable_styling(full_width = FALSE, position = "left", font_size = 11) %>%
  add_header_above(c(" " = 1, "Sample size" = 7)) %>%
  row_spec(0, bold = TRUE) %>%
  kable_classic(full_width = FALSE)
