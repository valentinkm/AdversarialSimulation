...
Data were generated based on a 5-factor population structural model with three indicators for each factor. Four different models were simulated (see Figure 3). In line with @rosseel_structural_2022, this model design was chosen to represent a realistic model with sufficient complexity to pose a challenge for the estimation methods, especially in the presence of misspecifications:
...
For both studies, convergence rates were tracked via lavaan's built-in function, which indicates convergence. Further, improper solutions—converged models that showed negative variances (as the only type of improper solution present)—were tracked via lavaan warning messages. Next, for all converged and proper solutions, bias and RMSE were computed using the following equations [@megha_joshi_2024].
