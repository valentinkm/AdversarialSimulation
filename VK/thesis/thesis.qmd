...
cat(paste0("Generated using the state of [AdversarialSimulation](https://github.com/valentinkm/AdversarialSimulation) as of commit SHA:", latest_sha))


# Get LATEST_SHA and LK_SHA from environment variables
latest_sha <- Sys.getenv("LATEST_SHA")
lk_sha <- Sys.getenv("LK_SHA")

# Check if the environment variables are set correctly
cat("#### Document Version: \n")
if (nzchar(latest_sha)) {
  cat(paste0("Generated using the state of [AdversarialSimulation](https://github.com/valentinkm/AdversarialSimulation) as of commit SHA:", latest_sha, "\n"))
} else {
  cat("LATEST_SHA is not set.\n")
}
...
cat(paste0("\\textit{(Git commit SHA ", file_sha, "):}\n\n"))
```
