...
```{=latex}
\vspace*{\fill} % Fills up space until the bottom of the page
\noindent % Prevents indent
{\textbf{Document Version:} \\
This document was generated using the state of the repository as of commit SHA: `r Sys.getenv("LATEST_SHA")`.}
```
...
document_link <- Sys.getenv("DOCUMENT_LINK")

print(paste("File SHA:", file_sha))
print(paste("Document Link:", document_link))

if (nzchar(document_link)) {
  cat("#### Quoted verbatim from Kosanke's report\n")

  cat(paste0("\\textit{(Version identified by Git commit SHA of thesis.qmd: ", file_sha, ")}\n\n"))


  cat(paste0("\\href{", document_link, "}{Click here to view the document}\n\n"))


  cat(paste0("Direct link: \\url{", document_link, "}\n"))
} else {
  cat("Document link not available.\n")
}

print(paste("Debug - File SHA:", file_sha))
print(paste("Debug - Document Link:", document_link))
```
