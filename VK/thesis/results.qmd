...
*Note* This figure shows the convergence rate and the rate of improper solutions for the different methods and conditions in simulation study 1.
...
p_convergence_heatmap_study2 <- conv_rate2 %>%
  ...
  theme_bw() +
  theme(
    text = element_text(size = 10),  # Increase the overall text size
    axis.text.x = element_text(size = 8, angle = 30, hjust = 1),  # Reduce angle to improve readability
    ...
    strip.placement = "outside",  # Move facet labels outside to reduce crowding
    panel.grid = element_blank(),
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
...
plot_data_aggregated_study1 <- aggr_summary_study_1 %>%
  select(model_type, N, reliability, method, mean_Bias) %>%
  mutate(
    N = factor(N, levels = c(100, 400, 6400)),
    reliability = factor(reliability, levels = c(0.3, 0.5, 0.7)),
    method = factor(method, levels = unique(method)),
...
*Note.* This figure shows the aggregated bias (absolute values) of the estimated regression parameters for the different methods and conditions in simulation study 1.
...
p_heatmap <- plot_data_aggregated_study3 %>%
  ggplot(aes(x = N, y = model_type)) +
  geom_tile(aes(fill = mean_Bias), color = "white") +
  geom_text(aes(label = sprintf("%.2f", mean_Bias)), size = 2.5, color = "black") +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, 
