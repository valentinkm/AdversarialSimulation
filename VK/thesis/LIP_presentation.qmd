...
:::{.notes}
Karl Popper once famously described Science as the art of "systematic over simplification".

This term ironically and yet accurately describes the very basic cycle of empirical research, where we lay out general claims about the world as hypotheses, translate them into measurable constructs and choose how to gather data from certain populations that finally in turn updates our beliefs about general verbal claims about the world.
:::
...
This mapping seems the crux of most research and here most, if not all of of a researcher's degrees of freedom lie. This makes this mapping the source of ambiguity and persistant disputes. When divergences in mappings from the verbal plane are not recognized or not transparent, persistent and seemingly unresolvable disagreements in the general and verbal plane occur.
:::
...
:::{.notes}
This challenge transfers to monte carlo simulation studies. These are commonly used tools to test statistical methods in simulated data that can be used to evaluate any method against a known ground truth.

As it is impossible to simulate and test every possible data and analysis model combination, here too, researchers are confronted with a multitude of degrees of freedom and decisions about what "prototypical" models to test in which "prototypical" data and settings.

Here, especially the comparison of different methods in their "general" applicability and performance for various research settings is prone to conflicting verbal claims based on diverging simulation decisions. Biases for a specific method developed by one researcher might additionally amplify these divergencies not only at the step of interpreting results but importantly also when designing a simulation.
:::
...
:::{.notes}
The basic idea is for two researchers in disagreement to first identify a general verbal dispute and agree on a research question to settle the debate.

Based on this they collaboratively work on operationalizing, testing and interpreting this verbal claim. This process aims to unveil and concretize underlying disagreements and thus reduce ambiguity and increase generalizability.

In this project we aim to transfer the concept of adversarial collaboration from the empirical domain to monte carlo simulation studies and assess its feasibility and viability in a case study in this context.
:::

## Creating an Adversarial Simulation <span style="color:#4682B4;">Framework</span>{auto-animate=true}

:::{.notes}
To conduct such an examplatory adversarial collaboration we first need a framework that structures the collaborative process tailored to the outline of simulation studies.
:::
...
:::{.notes}
As basis for this we first identified the core steps of a simulation study at which critical and distinctive decision points occur.
:::
...
:::{.notes}
This led us to a specific adversarial simulation framework. Importantly we decided to structure the collaboration in two rounds. In the first round, each collaborator works on the simulation study independently. In the second round, they collaborate on a joint study based on the results of the first round
:::

## Strucutral after Measurement (SAM) vs traditional SEM estimation <span style="font-size: 20px;"> Background</span>

:::{.notes}
Now, to set the stage for our specific case study I briefly outline the background of Structural After Measurement (SAM) as potential alternative to traditional Structural Equation Modeling (SEM) estimation.
:::
...
:::{.notes}
Traditional SEM methods, like maximum likelihood estimation, optimize all parameters simultaneously under the assumption of multivariate normality. While powerful, they often face issues such as non-convergence, improper solutions, and biases from local measurement misspecifications that affect the entire model. They also typically require large sample sizes for optimal performance.
:::
...
:::{.notes}
SAM addresses some limitations of SEM by separating the estimation into two phases. First, the measurement model parameters are estimated, either locally for each block or globally. Then, these fixed parameters are used in the structural model estimation. This approach aims to reduce the propagation of bias from the measurement to the structural part and decrease convergence issues, especially in smaller samples.
...
:::{.notes}
There have been conflicting reports on the performance of SAM versus SEM. Some studies suggest that SAM outperforms SEM in terms of convergence, bias, and RMSE, particularly in challenging conditions. Others find that SAM doesn't generally outperform SEM and that any apparent advantages may be due to bias cancellation under certain misspecifications. This disagreement forms the basis for our adversarial simulation case study.
:::
...
:::{.notes}
A replication of these conflicting studies represent round 1 of individually conducted studies.
:::
...
- "Toy" case study for AC<br>
  ...
  partly resolvable by thorough review <br>
  <br>
:::
:::{.fragment}
