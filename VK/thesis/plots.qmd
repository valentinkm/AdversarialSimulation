...
analyze_differences <- function(data) {
  ...
  # Calculate differences for SAM methods
  differences <- processed_data %>%
    filter(method %in% c("gSAM", "lSAM-ML")) %>%
    left_join(sem_ref, by = c("model_type", "N", "reliability")) %>%
    mutate(
      bias_diff = mean_Bias - sem_bound_bias,
      rmse_diff = mean_RMSE - sem_bound_rmse
    )
...
p_bias <- ggplot(differences, 
  ...
  labs(
    x = "",
    y = "Bias Difference (SAM - SEM-bound)",
    fill = "Method"
  ) +
...
p_rmse <- ggplot(differences, 
  ...
  labs(
    x = "",
    y = "RMSE Difference (SAM - SEM-bound)",
    fill = "Method"
  ) +
  facet_wrap(~model_type, scales = "free_x") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40")
...
summary_stats <- differences %>%
  group_by(model_type, method) %>%
  summarize(
    mean_bias_diff = mean(bias_diff),
    mean_rmse_diff = mean(rmse_diff),
    bias_beyond_threshold = sum(abs(bias_diff) > 0.01),
    total_conditions = n(),
    .groups = "drop"
  ) %>%
  arrange(model_type, mean_bias_diff)
```