...
```{r, include=FALSE}
# Aggregated Relative Bias for Study 2 (excluding misspecified paths)
...
```{r, include=FALSE}
# Parameterwise bias for study 3
data_plot <- plot_data_study_3
...
# parameterwise RMSE for study 3

fig_rmse_parameterwise_study3 <- data_plot %>%
  ggplot(aes(x = N, y = parameter)) +
  geom_tile(aes(fill = RMSE), color = "white") +
  geom_text(aes(label = sprintf("%.2f", RMSE)), size = 1.5, show.legend = FALSE) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, 
                       limits = c(0, 0.5), 
                       oob = scales::squish) +
  ggh4x::facet_nested(model_type + reliability ~ method, scales = "free", space = "free", switch="y") +
...
```{r, include=FALSE}
# Aggregated Bias for Study 3
fig_bias_aggregated_study3 <- plot_data_aggregated_study3 %>%
  ggplot(aes(x = N, y = model_type)) +
  geom_tile(aes(fill = mean_Bias), color = "white") +
...
# Aggregated RMSE for Study 3
fig_rmse_aggregated_study3 <- plot_data_aggregated_study3 %>%
  ggplot(aes(x = N, y = model_type)) +
  geom_tile(aes(fill = mean_RMSE), color = "white") +
  geom_text(aes(label = sprintf("%.2f", mean_RMSE)), size = 2.5, color = "black") +
  scale_fill_gradient2(low = "white", high = "red", limits = c(0, 0.5), oob = scales::squish) +
  ggh4x::facet_nested(reliability ~ method, scales = "free", space = "free", switch = "y") +
  theme_bw() +
  theme(
