...
params <- expand.grid(
  ...
  method = c("SEM", "SAM"),  # Include methods as part of the grid
  rep = 1:n_reps
) %>%
...
run_complete_simulation_study <- function(params, true_values) {
  ...
  # Internal function to evaluate model performance
  ...
  results <- future_pmap(params, function(model_type, N, reliability, method, rep, id) {
    ...
    if (!is.null(fit_result$fit) && fit_result$converged) {
      ...
      c(performance_metrics, Converged = 1)  # Use numeric for easy mean calculation
    } else {
...
simulation_results <- run_complete_simulation_study(params, true_values)
```
