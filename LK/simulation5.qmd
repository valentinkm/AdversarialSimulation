...
```{r}
get_dgm <- function(cl_condition, phis) {
  ...
  delta2 <- ifelse(cl_condition > 1, 0.4, 0)  # δ2 is 0.4 if cl_condition > 1
  delta3 <- ifelse(cl_condition > 2, 0.4, 0)  # δ3 is 0.4 if cl_condition > 2
  ...
  if (cl_condition >= 1) LAM[1, 2] <- delta1  # Cross-loading from F2 to F1
  if (cl_condition >= 2) LAM[4, 3] <- delta2  # Cross-loading from F3 to F2
  if (cl_condition == 3) LAM[7, 1] <- delta3  # Cross-loading from F1 to F3
  ...
  THETA <- diag(rep(1, 9))  # Assuming unique variances are all initially set to 1
