...
get_dgm <- function(psi_value, phis) {
  ...
  diag(PSI) <- 1 - diag(TR)  # Adjust for unique variances, assuming total variance of 1 for each item
  
  # Directly set the specified residual correlations for the interested pairs
  # Pair (X1, X4)
...
simulate_data <- function(N, psi_value, phis) {
  ...
  # Calculate the total covariance matrix (S) as the sum of trait (factor) and unique variances
  # Trait covariance matrix (TR) is LAM %*% PHI %*% t(LAM)
  TR <- LAM %*% PHI %*% t(LAM)
  S <- TR + PSI
