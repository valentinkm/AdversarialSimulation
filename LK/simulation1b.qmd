...
setup_design <- function() {
  # Sample sizes
  N_sizes <- c(50, 100)  # Including both N = 50 and N = 100
...
get_dgm <- function(lambda, phi) {
  ...
  LAM[1:3, 2] <- 0  # Setting off-diagonal elements to 0 for the first three rows
  LAM[4:6, 1] <- 0  # Setting off-diagonal elements to 0 for the last three rows
  
  # Factor correlation matrix
  PHI <- matrix(c(1, phi, phi, 1), nrow = 2, ncol = 2)
  
  # Error variances for each manifest variable
  error_variances <- 1 - lambda^2  # Assuming error variances are derived from 1 - lambda^2
  THETA <- diag(c(rep(error_variances, 6)))  # Diagonal matrix with error variances
  
  # In this focused study, residual correlations are directly specified between X1 and Y1, as well as X2 and Y2
  THETA[1, 4] <- THETA[4, 1] <- 0.12  # Residual correlation between X1 and Y1
  THETA[2, 5] <- THETA[5, 2] <- 0.12  # Residual correlation between X2 and Y2
...
simulate_data <- function(N, lambda, phi) {
  # Get DGM parameters adjusted for lambda and phi
  dgm_params <- get_dgm(lambda, phi)
  LAM <- dgm_params$LAM
  PHI <- dgm_params$PHI
  THETA <- dgm_params$THETA
  
  # The covariance matrix S is calculated as LAM %*% PHI %*% t(LAM) + THETA
  S <- LAM %*% PHI %*% t(LAM) + THETA
  rownames(S) <- colnames(S) <- c(paste0("X", 1:3), paste0("Y", 1:3))
  
  # Generate data using the MASS::mvrnorm function
...
report_analysis <- function(results_df) {
  ...
  for (model in model_types) {
    for (N in N_values) {
      # Filter results for the current model and sample size
      subset_df <- results_df %>%
        filter(ModelType == model, N == N)

      # Pivot the data to wide format to show Bias as a function of Lambda and Phi
      wide_df <- subset_df %>%
        pivot_wider(names_from = Phi, values_from = AvgAbsBias, names_prefix = "Phi_")

      # Order columns if necessary (Lambda, then Phi values)
      ordered_cols <- c("Lambda", sort(setdiff(names(wide_df), "Lambda")))
      wide_df <- wide_df[ordered_cols]

      # Generate a table name and store the table in the list
      table_name <- paste(model, "N", N, sep = "_")
...
Paper contradiction handling: 2 correctly specified rc, or none?
As of now, 2 as the paper says/ hints so multiple times, whereas only table 4 says otherwise.
