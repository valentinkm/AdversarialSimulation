...
#Calculate Bias
results_with_metrics <- results_long %>%
  group_by(N, lambda, phi, method_metric) %>%
  summarise(
    abs_bias = mean(abs(value - phi)),  # Calculate the mean of absolute bias
    .groups = "drop"  # Drop grouping after summarise
  )

# Split metrics by N into separate lists
split_metrics <- split(results_with_metrics, results_with_metrics$N)
...
process_each_N <- function(data) {
    ...
    estimator_splits <- data %>%
      ...
      map(~ .x %>%
           select(-method_metric,-N) %>%
           pivot_wider(names_from = phi, values_from = abs_bias, names_prefix = "phi_"))
    
    return(estimator_splits)
  }
...
ML_50 <- metrics_list$"50"$LSAM_ML
ULS_50 <- metrics_list$"50"$LSAM_ULS
ML_100 <- metrics_list$"100"$LSAM_ML
ULS_100 <- metrics_list$"100"$LSAM_ULS
