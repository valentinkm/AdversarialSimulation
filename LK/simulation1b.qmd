...
results_metrics <- results_df_raw %>%
  ...
  split_metrics <- results_metrics %>%
    group_by(N) %>%
    group_split() %>%
    set_names(map(., ~paste(unique(.x$N))))
  
  split_metrics2 <- split_metrics[["50"]] %>%
    select(starts_with("LSAM_")) %>%
    group_split() %>%
    set_names(map(., ~paste(unique(.x$N))))
  ...
  transform_group <- function(df_group) {
    df_group <- df_group %>%
      select(-N) %>%
      pivot_longer(cols = starts_with("LSAM_"), names_to = "method_metric", values_to = "value")
