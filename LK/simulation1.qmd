...
results_bias <- results_df %>%
  ...
  summarize(across(everything(), ~mean(abs(.x - phi))), .groups = 'drop')


extract_results <- results_df %>%
  group_by(N, rc, psi_value) %>%
...
list_results <-list(
  bias = extract_results %>% select(N, rc, psi_value, matches("_bias$")),
  sd = extract_results %>% select(N, rc, psi_value, matches("_sd$")),
  rmse = extract_results %>% select(N, rc, psi_value, matches("_rmse$"))
)
