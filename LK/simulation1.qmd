...
# Report analysis
```{r}
report_analysis <- function(abs_bias, ci_lower, ci_upper) {
  # Create a new tibble with formatted strings for each sample size column
  combined_df <- abs_bias %>%
  ...
  # Select only the formatted columns for a cleaner output
  formatted_columns <- combined_df %>%
    select(ends_with("formatted"))

  # Bind the method metric for reference
  final_result <- bind_cols(abs_bias %>% select(method_metric), formatted_columns)
    names(final_result) <- sub("_formatted$", "", names(final_result))

  return(final_result)
}

# Apply the function
bias_ci <- report_analysis(tables_list$"0_0.12"$abs_bias,tables_list$"0_0.12"$ci_lower, tables_list$"0_0.12"$ci_upper)
...
for (condition in names(tables_list)) {
  ...
  bias_ci_list[[condition]] <- report_analysis(tables_list[[condition]]$abs_bias,
                                               tables_list[[condition]]$ci_lower,
                                               tables_list[[condition]]$ci_upper)
}
