...
set.seed(1)
reps <- 2
seeds <-  purrr::accumulate(seq_len(reps - 1),
                            \(s, x)parallel::nextRNGStream(s), # x is disgarded, only the prior seed is relevant.
                            .init = .Random.seed)
```
...
simulation_study_ <- function(design){
  all_steps <- mutate(design, !!!future_pmap_dfr(design, planned_analysis, .options = furrr_options(seed = seeds)))
  all_steps
}

simulation_study <- function(design, k, seed = seeds) {
...
#Run simulation
results_sim <- simulation_study(design, 2, seeds)
