...
libraries <- c("GPArotation", "CDM", "miceadds", "TAM", "sirt", "lavaan", "dplyr", "tidyr", "purrr", "tidyverse")
...
apply_models <- \(d) map(estimators, exec, d)

d <- map(1:6, ~rnorm(100)) 
names(d) <- c(str_c("X", 1:3), str_c("Y", 1:3))
d <- as.data.frame(d)

planned_analysis <- compose(apply_models, simulate_data)

planned_analysis(100, 0, -.16)
```
...
design <- setup_design()
results_df <- mutate(design, !!!pmap_dfr(design, planned_analysis))
```
