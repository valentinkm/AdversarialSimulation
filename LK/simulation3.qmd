...
get_dgm <- function() {
  ...
  LAM[1:3, 1] <- lam1  # Loadings for X1 to X3 on Factor FX
  LAM[4:6, 2] <- lam2  # Loadings for Y1 to Y3 on Factor FY
...
simulate_data <- function(N) {
  # Use the get_dgm function to get the model parameters
  dgm_params <- get_dgm()
  LAM <- dgm_params$LAM
  PHI <- dgm_params$PHI
  THETA <- dgm_params$THETA
  
  # Compute the covariance matrix S based on the model parameters
  S <- LAM %*% PHI %*% t(LAM) + THETA
  rownames(S) <- colnames(S) <- c(paste0("X", 1:3), paste0("Y", 1:3))
  
  # Generate multivariate normal data with the MASS package
  dat <- MASS::mvrnorm(n = N, mu = rep(0, 6), Sigma = S)
  df_dat <- as.data.frame(dat)
  names(df_dat) <- c("X1", "X2", "X3", "Y1", "Y2", "Y3")
  
  # Return a list containing the sample size, simulated data, and misspecification parameters
  return(list(SampleSize = N, Data = df_dat))
}
...
planned_analysis <- function(N, model, model_type) {
  dataset <- simulate_data(N)$Data
...
```
Might there be NA's somewhere that results in the warning? Nothing visible in the results.
...
- Warnung: no non-missing arguments to min; returning Inf after simulation conduction?
