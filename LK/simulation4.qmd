...
report_bias <- function(metrics_list) {
  ...
  # Create an empty list to store results by DGM
  results_by_dgm <- list()
  ...
  for (dgm in unique_dgms) {
    # Filter metrics_list for the current DGM
    filtered_data <- metrics_list[metrics_list$DGM == dgm, ]
    
    # Initialize an empty data frame for storing bias results for the current DGM
    n_values <- unique(filtered_data$N)
    col_names <- c("Estimator", paste("N", n_values, sep = "_"))
    dgm_df <- setNames(data.frame(matrix(ncol = length(col_names), nrow = length(estimators))), col_names)
    dgm_df$Estimator <- sub("_metrics", "", estimators)
    
    # Iterate over each estimator
    for (estimator_idx in seq_along(estimators)) {
      estimator <- estimators[estimator_idx]
      
      # Iterate over each N
      for (n in n_values) {
        row_data <- filtered_data[filtered_data$N == n, ]
        metrics <- row_data[[estimator]][[1]]  # Assuming single row per N in filtered data
        
        # Format bias and CI
        formatted_bias <- sprintf("%.3f [%.3f, %.3f]", metrics$abs_bias, metrics$ci_lower, metrics$ci_upper)
        col_name <- paste("N", n, sep = "_")
        dgm_df[estimator_idx, col_name] <- formatted_bias
      }
    }
    
    # Store the data frame in the results_by_dgm list, named by the DGM value
    results_by_dgm[[paste("DGM", dgm, sep = "_")]] <- dgm_df
  }
  
  return(results_by_dgm)
}
