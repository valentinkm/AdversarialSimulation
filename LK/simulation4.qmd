...
simulation_study <- function(design, model, num_repetitions, true_phis, models) {
  ...
  for(model_type in models) {
    for(i in 1:nrow(design)) {
      ...
      rmse_list <- vector("list", length = num_repetitions)

      for(rep in 1:num_repetitions) {
        estimates_list <- planned_analysis(row$N_sizes, row$DGM_types, model, model_type)
        
        # Calculate biases and RMSE for each phi and store them
        ...
        rmse <- sapply(names(true_phis), function(phi) {
          if (!is.null(estimates_list[[phi]])) {
            sqrt(mean((estimates_list[[phi]] - true_phis[phi])^2))
          } else {
            NA  # Handle missing estimates
          }
        })
      ...
      # Flatten the lists to vectors and calculate AvgAbsBias and RMSE for the current condition
      all_biases <- unlist(biases_list, use.names = FALSE)
      avg_abs_bias <- mean(all_biases, na.rm = TRUE)
      
      all_rmse <- unlist(rmse_list, use.names = FALSE)
      avg_rmse <- mean(all_rmse, na.rm = TRUE)
