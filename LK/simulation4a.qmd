...
report_bias <- function(metrics_list) {
  ...
  for (estimator in estimators) {
    ...
    for (dgm in unique_dgms) {
      ...
      filtered_data <- subset(metrics_list, DGM == dgm)
      
      # Create a data frame for the current estimator and DGM
      df <- data.frame(beta = unique_betas)
      
      # Iterate over each N and beta
      for (n in sort(unique(filtered_data$N))) {
        beta_column <- sapply(unique_betas, function(beta) {
          row_data <- subset(filtered_data, N == n & beta == beta)
          if (nrow(row_data) > 0) {
            metrics <- row_data[[estimator]][[1]]  # Assuming single row per N and beta in filtered data
            sprintf("%.3f [%.3f, %.3f]", metrics$abs_bias, metrics$ci_lower, metrics$ci_upper)
          } else {
            NA  # or appropriate placeholder
          }
        })
        
        # Append the column for this N size
        df[[paste("N", n, sep = "_")]] <- beta_column
      }
      
      # Name and store the data frame for this DGM
