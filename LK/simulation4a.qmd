...
extract_results <- function(results_df_raw) {
  ...
  results_metrics <- function(values, beta) {
    adjusted_values <- values - beta  # Adjusting values by subtracting the scalar beta
    list(
      abs_bias = mean(abs(adjusted_values)),  
      rel_bias = mean(values - phi) / phi,
      rmse = sqrt(mean(adjusted_values^2)),
      se_bias = sd(adjusted_values) / sqrt(length(values)),
      ci_lower = mean(adjusted_values) - qt(0.975, df = length(values) - 1) * sd(adjusted_values) / sqrt(length(values)),
      ci_upper = mean(adjusted_values) + qt(0.975, df = length(values) - 1) * sd(adjusted_values) / sqrt(length(values))
    )
  }
...
report_bias <- function(metrics_list) {
    # List of estimators to process
    estimators <- c("SEM_ML_metrics", "SEM_ULS_metrics", "LSAM_ML_metrics")
    
    # Create an empty list to store results by estimator and DGM
    results_by_estimator_and_dgm <- list()
    ...
    # Process each estimator and DGM
    for (estimator in estimators) {
        estimator_dgm_list <- list()
        
        for (dgm in unique_dgms) {
            # Filter metrics_list for the current estimator and DGM
            dgm_filtered_data <- subset(metrics_list, DGM == dgm)

            # Get unique N values sorted
            unique_ns <- sort(unique(dgm_filtered_data$N))
            
            # Prepare a data frame with beta as the first column and one column for each N value
            df <- setNames(data.frame(matrix(ncol = length(unique_ns) + 1, nrow = length(unique_betas))), c("beta", paste("N", unique_ns, sep = "_")))
            df$beta <- unique_betas
            
            # Fill the data frame
            for (beta in unique_betas) {
                for (n in unique_ns) {
                    beta_filtered_data <- subset(dgm_filtered_data, N == n & beta == beta)
                    col_name <- paste("N", n, sep = "_")
                    
                    if (nrow(beta_filtered_data) > 0) {
                        metrics <- beta_filtered_data[[estimator]][[1]]
                        df[df$beta == beta, col_name] <- sprintf("%.3f [%.3f, %.3f]", metrics$abs_bias, metrics$ci_lower, metrics$ci_upper)
                    } else {
                        df[df$beta == beta, col_name] <- NA  # Placeholder if no data is available
                    }
                }
            }
            
            # Store the data frame for this DGM
            estimator_dgm_list[[paste("DGM", dgm, sep = "_")]] <- df
        }
        
        # Store the list of DGM-specific data frames under the current estimator
        results_by_estimator_and_dgm[[estimator]] <- estimator_dgm_list
    }
    
    return(results_by_estimator_and_dgm)
}
