...
report_bias <- function(metrics_list) {
  ...
  # Combine results into separate dataframes for each estimator
  results_by_estimator <- map(estimators, function(estimator) {
    map_dfr(results_by_dgm, ~ .x[[estimator]])
  })
  
  set_names(results_by_estimator, estimators)
}
...
#Report Bias does not work yet
bias_ci <- report_bias(metrics_list)
bias_ci

```
bias_ci outputs across DGM's 1 and 2, which are not part of the nesting, yet.
