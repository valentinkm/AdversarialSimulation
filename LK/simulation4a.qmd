...
This time, we will define the DGM the same way RL did:
```{r}
...
# Simulate data
Analogously to RL, we will simulate the data with the lavaan function, for each condition.
Here, the matrices_and_simulateData.r should be inserted in a way that allows it to be varied for each condition later on.
...
simulate_data <- function(N, DGM, beta) {
 NFAC   <- 5L    # number of factors
  ...
  return(list(SampleSize = N, DGM = DGM, beta = beta, Data = df_dat))
}
...
planned_analysis <- function(N, DGM, beta, model, model_type) {
  dataset <- simulate_data(N, DGM, beta)$Data
  ...
  for (model_type in models) {
    for (i in 1:nrow(design)) {
      ...
      for (rep in 1:num_repetitions) {
        ...
        # Directly use row$beta_Sizes as the single true value for bias calculation
...
SEM_CL1 <- list_of_tables[["SEM_ULS_CL_1"]] # Example for Bias as a function of N and Phi with 1 cross-loading
...
lsam_table <- list_of_tables[["LSAM_ULS_CL_3"]]
sem_table <- list_of_tables[["SEM_ULS_CL_3"]]
...
Why exactly this kind of estimation methods?? Also for simulation 4
