...
To facilitate a structured comparison and integration of studies throughout the collaboration process, we agreed on a framework for conducting our individual simulation studies in advance, that is visible in Table 1.
...
|      | - Determine factors to vary, levels, and whether fully, partly factorially, or one at a time. |
|      | - Examples include: |
|      |   - Sample size |
...
As simulation studies are computationally intensive and require a structured approach to save, share and review code, we propose using Github to ease collaboration.
...
These can even be assigned a DOI via Zenodo.
...
As @robitzsch_comparing_2022 argued in their paper, and was shown already in other simulations, using constrained maximum likelihood estimation should resolve convergence issues of classical maximum likelihood estimation in smaller samples.
...
This was true for both relative bias and RMSE, and visible for the former in Table 2.
...
Table 3 shows this finding in Study 3, in conditions with both one unmodelled residual correlation and one cross-loading.
...
Table 5 shows this was especially apparent in a sample size of 50.
```{r eval = TRUE, echo = FALSE}
...
kbl(bias_ci_s1b[["N_50"]][["LSAM_ML"]], 
  ...
  add_header_above(c("Study 1b: Absolute Bias of LSAM-ML for N=50" = 6),
                   bold = TRUE, align = "l", line = TRUE, italic = FALSE)
...
Table X shows the performance of SEM-ML, whereas Table Y shows the performance of LSAM-ML in DGM 2 (in presence of unmodelled cross-loadings).
...
Additionally, the performance differences observed disappeared, when accounting for SAM's negative small sample bias.

In alignment with what Kosanke observed in his simulations, Kriegmair's results also hint at the present of SAM's negative small sample bias, as the largest performance differences were observed in conditions of low reliability, i.e lambdas, where the bias is especially pronounced.

The evident performance differences observed between traditional SEM and SAM estimation in the study by Kriegmair are thus to be interpreted differently for all effects.
This is true for all performance measures, even for coverage, which was not investigated in the study by Kosanke.
Coverage is a measure that is strongly determined by bias.
For this reason, results aligned in terms of bias and coverage in the study by Kriegmair.
Thus, there is no reason to suspect that the choice of performance measure might impact the results observed in the study by Kosanke.
...
Other differences in the individual studies were the additional inclusion of effects with regards to endo- and exogeneity of predictors and the number of measurement blocks.
Findings were not showing a substantial impact of these conditions on the estimation results.

The difference of observing effects of regression weigths directly in the study by Kosanke, and total variance explained by Kriegmair, are also minor, as both are aimed at modifying the regression weigths and lead to comparable parameter values (PROOOF?).
But: Different effects! minimal impact of R-squared for Valentin, but effect of beta for me? (only in some conditions) --> What range were the regression values, in the end?
Additionally, both studies did not find substantial effects of this variation?

Another difference is the inclusion of conditions that examine the misspecification of the regression directionality (i.e. switching predictor and outcome variable incorrectly in the estimation model).
...
Already the first step of agreeing on joint research questions is a collaborative effort that requires to change perspective, and more deeply understand what the adversaries did in their studies.
...
Still, besides these assumptions and learnings from our first case study, we were unable to test the main part of collaboration and can not conclude that the practical applicability and technical feasibility is given.
