...
create_styled_table <- function(data, condition) {
  ...
  kbl(data,
    ...
    kable_styling(full_width = F, position = "left",
                  latex_options ="scale_down", "hold_position") %>%
...
kable(warn_s4, format = "latex", booktabs = TRUE, longtable = TRUE
      , escape = FALSE) %>%
...
kable(warn_s4a, format = "latex", booktabs = TRUE, longtable = TRUE
      , escape = FALSE) %>%
  kable_styling(latex_options = "hold_position", full_width = FALSE) %>%
  column_spec(1, width = "30em") %>%
  column_spec(2, width = "5em") %>%
...
kbl(bias_ci_s4a[["DGM_1"]][["SEM_ML_metrics"]], 
    col.names = c("beta", "N=50", "N=100", "N=250",
                  "N=500", "N=1000", "N=2500", "N=100.000")) %>%
  kable_styling(full_width = F, position = "left",
                latex_options ="scale_down") %>%
  add_header_above(c(" " = 1, "Sample size" = 7)) %>%
  row_spec(0, bold = TRUE) %>%
