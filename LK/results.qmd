...
Even though this bias is correctly identified and explained already in Study 1, @robitzsch_comparing_2022 conduct multiple follow-up studies only investigating positive values in unmodelled cross-loadings and residual correlation, namely studies 3-6. 
...
LSAM-estimation generally outperforms SEM-ULS estimation in smaller samples across all DGM's.
Moreover, LSAM- outperforms both SEM-estimators in smaller samples, when unmodelled cross-loadings are present.
...
For DMG 1, results are very similar, with LSAM-ML outperforming SEM-ULS estimation.
...
Thus, LSAM-ML clearly outperforms the two SEM-estimators for small to moderate samples sizes, in the conditions with four unmodelled cross-loadings.
...
LSAM-ML outperfoms the two SEM-estimators in conditions with unmodelled cross-loadings. 
...
Overall, the main performance difference is LSAM outperforming the two SEM-estimators in smaller to moderate samples sizes, in conditions with unmodelled cross-loadings (DGM1).
...
Thus, LSAM-ML generally outperformed SEM-estimation and SEM-ML outperformed SEM-ULS estimation, in the presence of four unmodelled cross-loadings.
...
Thus, none of the estimators generally outperforms another in the presence of 20 unmodelled residual correlations.
...
This effect is stronger for higher values of beta.
SEM-ULS had reduced RMSE values for higher N.
All three estimators have higher RMSE values for higher beta.
This effect was especially present in the two SEM estimators.

Thus, LSAM-ML generally outperformed SEM-estimation, in the present of 20 unmodelled residual correlation.
Even more so, for higher values of beta.


## Summary Study 4a

In terms of Bias, only in the presence of four unmodelled cross-loadings, did LSAM-estimation generally outperform both SEM-estimators.
This was especially evident for higher beta values.

With regards to RMSE, LSAM-ML outperformed SEM-estimation both under unmodelled cross-loadings and residual correlations.
The same effect with regards to higher beta values was present in conditions with unmodelled residual correlation.

All in all, this indicates that LSAM-ML estimation generally outperforms SEM-estimation in data-generating scenarios that contain regressions, especially if the regression coefficients have lower to moderate values.
...
As we did not find effects specific for comparing the estimators with regards to increasing N (besides the obvious decrease in Bias and RMSE), we can still compare the results in a general manner.
...
Also no differential effect for higher beta values in original paper.
...
Still, later on in the paper, they agree that SAM can be expected to be more robust than both SEM-estimators in models with non-saturated structural models.
With this last conclusion, we agree and extend the results to be even more convincing in that regard.

# Summary for 5-factor-model under 3 different data-generating mechanisms (Studies 4 and 4a)

For low beta (0.1), LSAM outperformed the two SEM-estimators in smaller to moderate samples sizes, in conditions with unmodelled cross-loadings (DGM1).
In conditions without misspecification (DGM 0) and in the presence of unmodelled residual correlations (DGM 2), there is no general advantage of one estimation method over another.

With increasing values of beta (0.2-0.4), the effect with regards to unmodelled cross-loadings became even stronger (DGM 1).
in this context, LSAM not only outperformed SEM-estimation in terms of efficiency in DGM1, but also in conditions with unmodelled residual correlations (DGM2).

All in all, this indicates that LSAM-ML estimation mostly outperforms SEM-estimation in data-generating scenarios that contain regressions, especially if the regression coefficients have increasing values.
...
With regards to 2-factor-CFA-models, as in the original paper by @robitzsch_comparing_2022, SAM does not generally outperform SEM in small to moderate samples. 
SAM exhibits a negative small sample bias that makes SAM appear superior in conditions with unmodelled positive cross-loadings and residual correlations.
This bias is especially strong for lower lambda and higher phi values.
If there is no misspecification or unmodelled negative cross-loadings and residual correlations, SAM tends to perform even worse than traditional SEM.

On the other side, when structural models are unsaturated, and thus contain regressions, LSAM is more robust than SEM-estimation to misspecifications, if the regressions are of smaller to moderate size and the values of the unmodelled parameters are positive.
Only in the presence of unmodelled cross-loadings, this is generally the case for low beta-values as well.

On the other side, when structural models are unsaturated, and thus contain regressions, the picture looks similar:
All in all, this indicates that LSAM-ML estimation mostly outperforms SEM-estimation in data-generating scenarios that contain regressions, especially if the regression coefficients have increasing values.
...
Only if this can be ruled out, we could conclude that SAM generally outperforms SEM in these models.
...
-------------------To delete:
- Explicitly capture warnings/ non-convergence
- From Simulation1 and 2: Investigate conditions with higher number of misspecifications (Study4?) -> No outperformance of SAM for 20 rc!
- conditions with negative unmodelled cross-loadings or residual correlations should be included for unsaturated structural models.
