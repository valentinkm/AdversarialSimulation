...
We did a mistake with the calculation of the confidence intervals, as we did not include the absolute, but relative mean in the two limits. 
Also, the standard error was wrongly calculated as relative, as well:
Thus, we have to reextract the results correctly:
```{r}
#Load raw results

results_df_raw <- readRDS("SimulationResults/sim1b_results_raw.rds")
# Extract results
...
bias_ci_s1b <- suppressMessages(report_bias(metrics_list))
