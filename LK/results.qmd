...
Importantly, the model parameters and resulting population-level covariance matrices for the different DGM's provided by @robitzsch_comparing_2022 in their github repository were not the same as in the first paper by @rosseel_structural_2022. Combined with the facts that firstly, the values at some point seemed to be off in @robitzsch_comparing_2022 (they included for example a fixed loading of 0 for y11 in DGM2, without an apparent reason), and secondly that only in @rosseel_structural_2022 the calculation of the values was replicable, we chose to use the values of @rosseel_structural_2022 by generating them with the same functions they used.
...
Ideally, beides reporten, eines interpretieren, bei mir dann rel_bias wo möglich/ nötig
...
Noch einmal das Repo: https://github.com/aaronpeikert/varynfit/blob/master/varynfit.Rmd angucken
