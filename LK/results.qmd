...
For these reasons, and because of the fact that the calculation of the values in @rosseel_structural_2022 was replicable, we chose to use the values of @rosseel_structural_2022 by generating them with the same functions they used. 
...
In their paper, what @robitzsch_comparing_2022 called DGM1 is implemented with a value of 0. 
DGM 2 and 3 are thus also implemented with values 1 and 2. 
To avoid confusion, I will use the terminology of the code from @rosseel_structural_2022. 
Thus, the naming is different when interpreting the results later on.
...
Additionally, we will only use the term "generally" to refer to the first meaning, that it outperforms across multiple data generating scenarios or conditions.
...
If, however, there are differences large than 0.03, we will report them.
...
As the 245 are less than 0.5% of the 525000 estimations performed, we will continue with the interpretation as planned, as their impact is negligible.
...
for (condition in names(bias_ci_s1)) {
  create_styled_table(bias_ci_s1[[condition]], condition)
...
for (condition in names(sd_s1)) {
  create_styled_table(sd_s1[[condition]], condition)
  cat("\n\n")  # Add space between tables
}
```
...
The combined study should check this more properly to evaluate these claims thoroughly.
As the 414 estimation are around 0.5% of the 75000 estimations performed, we will continue with the interpretation as planned, as their impact is negligible.
...
for (condition in names(bias_ci_s2)) {
  create_styled_table(bias_ci_s2[[condition]], condition)
  cat("\n\n")  # Add space between tables
}
...
for (condition in names(sd_s2)) {
  create_styled_table(sd_s2[[condition]], condition)
  cat("\n\n")  # Add space between tables
}
```
...
If there is no misspecification or unmodelled negative cross-loadings and residual correlations, SAM tends to perform worse than traditional SEM.
...
for (condition in names(bias_ci_s4)) {
  create_styled_table(bias_ci_s4[[condition]], condition)
  cat("\n\n")  # Add space between tables
}
...
Even though LSAM appears to outperform SEM-estimation in DGM 1, this might be attributable to its general negative small sample bias.
...
Even though LSAM appears to outperform SEM-estimation in DGM 1, this might be attributed to its general negative small sample bias, that persisted even for low phi values in our study1b.
Thus, it cannot simply be stated, that SAM estimation should be preferred in these scenarios.
...
Similarly as in Study 4, next to the warnings regarding the Gamma calculation, all 89 other warnings referred to problems with positive definite matrices or model identification.
This suggests potential issues with multicollinearity or redundancy among latent variables, but only in a negligible number of estimations.
The 9000 warnings with regards to Gamma computation in smaller samples is negligible as well, as we are not interested in the computation of robust fit indices.
...
With this last conclusion, we disagree for now with regards to the now multiple times mentioned potential negative small sample bias of LSAM, that appears to be present in non-saturated structural models as well.
...
This effect was even stronger for higher values of phi and low N, which aligns with the findings in our studies for models with regressions..
...
Going ahead of what was investigated in @robitzsch_comparing_2022, we found that this bias might also present in models with regressions.
