...
```
First 1000 entries no error & message

First 1000 entries quite some warnings, but all the same
$warnings[[8]], 14, 28[1 und 3], 41, 49, 53, 56, 63, 84[1 und 3], 95, 96, 98, 99, 116, 142[1 und 3], 162, 163, 173, 174, 181[1 und 3], 193, 195[1 und 3], 202[1,3,5],
-> "no non-missing arguments to min; returning Inf" "no non-missing arguments to min; returning Inf"

Might just mean that there are in fact, convergence issues in ml estimation, even though constrained ML estimation!
DEVIATION: Exclude casees with non-convergence -> Not possible anymore! Just count, report and guess.
...
Comparing these results with the original paper, the results are very similar and can be seen as succesfully replicated.
Thus, we draw the same main conclusions as Robitzsch (2022):
