...
- Unbiased judgement overall
Across all conditions, LSAM-ML is biased.

- ALL across increasing lambda
For higher values of lambda, LSAM-ML has less absolute Bias. This difference increases for higher phi values.

- ALL across increasing phi
For lower values of lambda, the absolute bias increases with increasing phi. This trend reverses for higher values of lambda, where the absolute bias decreases with higher phi values.
...
```
The values are nearly identical to LSAM-ML. 
Therefore, the interpretation is the same for LSAM-ULS

### N=100, LSAM-ML
```{r}
#| ouput: true

print(bias_ci_s1b[["N_100"]][["LSAM_ML"]])
print(bias_ci_s1b[["N_100"]][["LSAM_ULS"]])
...
Again, the interpretation is identical to N=50

### Comparison to original paper
In contrast to the original paper, we computed absolute bias to avoid infinte values in some conditions.
Still, 
