...
LSAM-ML substantially outperforms the two SEM-estimators in small samples (N=50-100).
...
Only SEM-ULS is unbiased for N=500 as well.
...
LSAM- outperforms SEM-ULS estimation in small samples (N=50) across all DGM's.
Moreover, LSAM- outperforms SEM-estimation in all conditions with unmodelled cross-loadings.
...
################One important point to discuss is again the exclusive presence of positive loadings and residual correlations in the DGM's 1 and 2. 
This might lead again to SAM's small sample bias make it appear to outperform. 
But: Is this not negligible here, as we have low (cross) loadings (0.42) and beta (/phi??) values (0.1)?? No CFA though!

#### Printing syntax again?

```{r}
#MLIST <- lav_sam_gen_model(nfactors = NFAC, nvar.factor = FVAR, lambda = lambda,
#                           PSI = PSI, BETA = BETA, reliability = REL, misspecification = 0L,rho = #0.6)
...
Weirdly, unlike @robitzsch_comparing_2022, we have found some bias in DGM 0 ??

For DMG1, results are very similar, with LSAM outperforming
