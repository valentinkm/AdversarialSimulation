...
#  Simulation Study
```{r}
simulation_study <- function(design, model, num_repetitions = num_repetitions, true_phi, models) {
  results <- list()
  
  for(model_type in models) {
    for(i in 1:nrow(design)) {
      row <- design[i, ]
      phi_estimates <- numeric(num_repetitions)
      
      for(rep in 1:num_repetitions) {
        phi_estimates[rep] <- planned_analysis(row$N_sizes, row$rc_conditions, row$psi_values, model, model_type)
      }
      ...
      results[[length(results) + 1]] <- c(ModelType = model_type, N = row$N_sizes, RC = row$rc_conditions, PsiValue = row$psi_values, AvgAbsBias = average_absolute_bias, SDPhi = sd_phi, RMSEPhi = rmse_phi)
    }
  }
